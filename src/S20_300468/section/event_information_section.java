...
public class event_information_section extends CommonParser {
    public void parse(byte[] descriptBuffer, int len) throws Exception {
        ...
        parseData("segment_last_section_number 8 uimsbf ");
        parseData("last_table_id 8 uimsbf ");
        {
            pushNode("events");
            int _position2_ = getBufferLen() - 4 /* 4 bytes CRC32 */;
            while (getToken() < _position2_)// N
            {
                pushElement();
                {
                    parseData("event_id 16 uimsbf ");
                    parseData("start_time 40 bslbf ");
                    parseData("duration 24 uimsbf ");
    ...
    public String getName() {
        return "event_information_section";
    }
