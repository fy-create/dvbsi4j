...
public class target_region_name_descriptor extends CommonParser {
    public void parse(byte[] descriptBuffer, int len) throws Exception {
        ...
        {
            pushNode("regions");
            int _position7 = getBufferLen();
            while (getToken() < _position7) // N
            {
                pushElement();
                {
                    parseData("region_depth 2 bslbf ");
                    parseData("region_name_length 6 bslbf ");
                    // Use dynamic context length parse buffer
                    parseBytes("text_char " + (contextGet("region_name_length")) * 8 + " uimsbf ");
