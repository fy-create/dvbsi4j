...
public class multilingual_component_descriptor extends CommonParser {
    public void parse(byte[] descriptBuffer, int len) throws Exception {
        ...
        {
            pushNode("components");
            int _position4 = getBufferLen();
            while (getToken() < _position4) // N
            {
                pushElement();
                {
                    parseData("ISO_639_language_code 24 bslbf ");
                    parseData("text_description_length 8 uimsbf ");
                    // Use dynamic context length parse buffer
                    parseBytes("text_char " + (contextGet("text_description_length")) * 8 + " uimsbf ");
                }
                popElement();
            }
            popNode("components");
