...
public class DTS_audio_stream_descriptor extends CommonParser {
    public void parse(byte[] descriptBuffer, int len) throws Exception {
        ...
        parseData("sample_rate_code 4 bslbf ");
        parseData("bit_rate_code 6 bslbf ");
        parseData("nblks 7 bslbf ");
        parseData("fsize 14 uimsbf ");
        parseData("surround_mode 6 bslbf ");
        parseData("lfe_flag 1 uimsbf ");
        parseData("extended_surround_flag 2 uimsbf ");
        // Use dynamic length parse buffer
        parseBytes("additional_info_byte " + (getBufferLen() - getToken()) * 8 + " uimsbf ");
    }

    public String getName() {
        return "DTS_audio_stream_descriptor";
    }
