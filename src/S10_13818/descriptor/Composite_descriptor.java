...
public class Composite_descriptor extends CommonParser {
    public void parse(byte[] descriptBuffer, int len) throws Exception {
        ...
        {
            pushNode("sub_descriptors");
            int descriptor_length_bytes = contextGet("descriptor_length") + getToken();
            while (getToken() < descriptor_length_bytes) // N
            {
                pushElement();
                {
                    parseData("sub_descriptor_tag 8 uimsbf ");
                    parseData("sub_descriptor_length 8 uimsbf ");
                    // Use dynamic context length parse buffer
                    parseBytes("private_data_byte " + (contextGet("sub_descriptor_length")) * 8 + " uimsbf ");
                }
                popElement();
            }
            popNode("sub_descriptors");
        }
    }
