...
public class dvb_j_application_descriptor extends CommonParser {
    public void parse(byte[] descriptBuffer, int len) throws Exception {
        ...
        parseData("descriptor_length 8 uimsbf ");
        {
            ...
            while (getToken() < descriptor_length_bytes) // N
            {
                pushElement();
                {
                    parseData("parameter_length 8 uimsbf ");
                    // Use dynamic context length parse buffer
                    parseBytes("parameter_byte " + (contextGet("parameter_length")) * 8 + " uimsbf ");
                }
                popElement();
            }
            popNode("parameters");
